        ...
        if (data.included) {
            for (const item of data.included) {
                ...
                if (item.publicIdentifier == username) {
                    ...
                    profileData.publicIdentifier = username
                }
            }
        }
    ...
    function traverse(obj) {
      ...
      for (const key in obj) {
        ...
        if (key === 'entityComponent') {
          ...
          if (entity?.titleV2?.text?.text) {
            item.title = entity.titleV2.text.text;
          }
          if (entity?.subtitle?.text) {
            item.subtitle = entity.subtitle.text;
          }
          
          // Extract data from subComponents
          if (entity?.subComponents?.components?.length > 0) {
            entity.subComponents.components.forEach(subComponent => {
              traverse(subComponent); // recursively process subcomponents
            });
          }
